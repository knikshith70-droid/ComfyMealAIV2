...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Rate limit: no more than one code per email per 60s
    ...
    if (!resendKey) {
      // Dev fallback: log the code so it can be read during testing.
      console.log(`[send-otp] No RESEND_API_KEY. OTP for ${normalized}: ${code}`);
      return new Response(
    ...
    const emailRes = await fetch(RESEND_URL, {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: normalized,
        subject: "Your ComfyMeal AI sign-in code",
  ...
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table width="420" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;border:1px solid #EFE7D6;padding:32px;">
      ...
      <tr><td style="text-align:center;padding-top:8px;padding-bottom:20px;color:#3A3530;font-size:15px;line-height:1.5;">
        Use the code below to sign in. It expires in ${CODE_TTL_MINUTES} minutes.
      </td></tr>
