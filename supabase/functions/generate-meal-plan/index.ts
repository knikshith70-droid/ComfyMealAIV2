...
function buildPrompt(body: RequestBody, days: number) {
  ...
  const system = `You are ComfyMeal AI, a practical meal-planning sous-chef. You design multi-day meal plans that use what the user actually has in their pantry and spice rack. You ALWAYS respond with ONLY raw JSON — no markdown, no code fences, no \`\`\`json wrappers, no <think> tags, no explanations, no introductory text, no trailing commentary.
      ...
      body: JSON.stringify({
        ...
        response_format: {
          ...
          json_schema: {
            ...
            strict: true,
            schema: MEAL_PLAN_RESPONSE_SCHEMA,
          },
        },
      }),
    });
...
function normalizePlan(parsed: unknown): DayPlan[] {
  ...
  if (Array.isArray(obj.days)) {
    ...
    throw new Error("LLM response did not contain a days array.");
  }
  const days: DayPlan[] = [];
  ...
  if (days.length === 0) {
    throw new Error("LLM response did not contain any valid days.");
  }
  return days;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const maxTokens = Math.min(65536, 8000 + days * 4000);
