...
- When generating multiple recipes, each MUST be fundamentally different:不同的烹饪技法 (different cooking techniques like stir-fry, roast, simmer, grill, raw/assembly), 不同的风味方向 (different flavor directions like tangy, savory, spicy, fresh/herby, rich/creamy), 不同的主要食材.
...
function normalizeRecipes(parsed: unknown): RecipeShape[] {
  ...
  if (Array.isArray(obj.recipes)) {
    ...
    throw new Error("Groq response did not contain a recipes array.");
  }
  ...
  if (recipes.length === 0) {
    throw new Error("Groq response did not contain any valid recipes.");
  }
  return recipes;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // More recipes + nutrition needs more tokens
    const maxTokens = count >= 4 ? 4000 : count >= 3 ? 2800 : 1800;
